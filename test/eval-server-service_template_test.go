...
func TestEvalServerServiceTemplateRenderedDeployment(t *testing.T) {
	t.Parallel()
	...
	options := &helm.Options{
		SetValues: map[string]string{
			...
			"els.service.type": "ClusterIp",
			"els.service.port": "5100",
		},
	...
	output := helm.RenderTemplate(t, options, helmChartPath, releaseName, []string{"templates/eval-server-service.yaml"})
	...
	require.Equal(t, "featbit-els", service.Name)
	...
	require.Equal(t, intstr.FromInt(5100), service.Spec.Ports[0].TargetPort)
	//if ingress 80 else 5000 or custom value
	require.Equal(t, int32(5100), service.Spec.Ports[0].Port)
