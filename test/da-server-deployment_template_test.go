...
func TestDaServerDeploymentTemplateRenderedDeployment(t *testing.T) {
	t.Parallel()
	...
	options := &helm.Options{
		SetValues: map[string]string{
			...
			"das.image.registry":      "docker.io",
			"das.image.repository":    "featbit/featbit-data-analytics-server",
			"das.image.pullPolicy":    "IfNotPresent",
			"das.image.tag":           "2.4.1",
			"das.autoscaling.enabled": "false",
		},
	...
	output := helm.RenderTemplate(t, options, helmChartPath, releaseName, []string{"templates/da-server-deployment.yaml"})
	...
	expectedContainerImage := "docker.io/featbit/featbit-data-analytics-server:2.4.1"

	assert.Equal(t, deployment.Namespace, namespaceName)
	require.Equal(t, deployment.Name, "featbit-das")
	...
	require.Equal(t, deploymentContainers[0].Name, "featbit-das")
	...
	require.Equal(t, deploymentContainers[0].LivenessProbe.HTTPGet.Path, "/api")
	...
	require.Equal(t, deploymentContainers[0].ReadinessProbe.HTTPGet.Path, "/api")
	...
	require.Equal(t, "CHECK_DB_LIVNESS", env[0].Name)
	require.Equal(t, "false", deploymentContainers[0].Env[0].Value)
